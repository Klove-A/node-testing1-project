...
class Car {
   ...
   */
  drive(distance) {
    const milesWeCanGo = this.tank * this.mpg
    if (distance <= milesWeCanGo) {
      this.odometer += distance;
      this.tank = this.tank - distance / this.mpg
    } else {
      this.odometer = this.odometer + milesWeCanGo
      this.tank = 0
    }
   ...
   */
  refuel(gallons) {
    const whatCanFit = this.tankSize - this.tank
    if (gallons <= whatCanFit) {
      this.tank += gallons
    } else {
      this.tank = this.tank + whatCanFit
    }
    return this.tank
  }
}
